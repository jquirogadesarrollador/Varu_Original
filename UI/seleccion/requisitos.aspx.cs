...
public partial class seleccion_requisitos : System.Web.UI.Page
{
    ...
    {
        Page.Header.Title = "VERIFICACIÓN DE DOCUMENTOS PARA CONTRATACIÓN";
    ...
    protected void Button_GUARDAR_Click(object sender, EventArgs e)
    {
        ...
        if (verificador == true)
        {
            ...
            {
                Informar(Panel_FONDO_MENSAJE, Image_MENSAJE_POPUP, Panel_MENSAJES, Label_MENSAJE, "No se encontraron documentos en la Grilla de Documentos!.", Proceso.Error);
            }
            else
            {
                Boolean cumplido = _requisito.CumplirSinDatosDeDerogacion(tablaDocumentos);

                if (cumplido)
                {
                    Informar(Panel_FONDO_MENSAJE, Image_MENSAJE_POPUP, Panel_MENSAJES, Label_MENSAJE, "La información de cumplimiento de documentos fue almacenada correctamente.", Proceso.Correcto);
    ...
    private void HabilitarFilasAptasEnGrillaDocumentos(GridView grilla)
    {
        for (int i   = 0; i < grilla.Rows.Count; i++)
        {
            ...
            if (filaGrilla.Cells[0].Text == "DOCUMENTO")
            {
                HabilitarFilaGrilla(grilla, i, 0);
                checkCumplido.Enabled = true;
            }
